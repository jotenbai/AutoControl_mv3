...
| Toolbar button editor (up to 8, icons, title) | ✅ | icon pipeline (`_Fh`/`_zr`/`_Ve`/`_Pk`; NOT `_6t`) re-implemented in sw.js (OffscreenCanvas) + startup push fixed 2026-08-08 (`_Pk` in configLoaded + button-side `TBBtnInit` retry, mh_test B29). `_6t` is gesture-display type 90 — §7-17 |
...
| 18 | Open URL `chrome://history` / bookmarks stuck on Loading or blank (gestures) | ✅ | **fixed 2026-09-11** — same Chrome WebUI-from-SW paint class as sessions.restore, **different API** (do NOT fold into that PR). Open URL (`_Mh` file8) → `_6a`/`_3g` (file34) → `chrome.tabs.create` / `windows.create` with `active:false`. Gesture vs hotkey use the SAME action runner; gestures additionally schedule RBTN-ESC (type 300 Esc, 20ms). `chrome://history` from a SW create often sticks on "正在加载…"; bookmarks sometimes blank; F5 always heals. `chrome://extensions` via Alt+E often paints — that does NOT prove keyboard-open-history would work. FIX: wrap `chrome.tabs.create` + `chrome.windows.create` in sw.js after importScripts; after create, if the URL is `chrome://` or `edge://` (skip `about:blank` and the new-tab page), `chrome.tabs.reload` once immediately. Gesture 750s that Open URL chrome:// are held ~35ms so RBTN-ESC (20ms) lands on the old tab first (Esc on the new WebUI → Loading… then blank; keyboard Alt+X has no Esc and works). Other gestures are not delayed.. https Open URL is not reloaded (would flash). `_Yk === chrome` in the SW so `_6a`/`_3g` pick up the wrap; page-proxy `tabsCreate`/`winsCreate` too. NO bundle rebuild. mh_test B55. |

| 17 | Gesture display (on-screen direction HUD) never shown | ✅ | **fixed 2026-09-11** — SW `_6t` used `!b.enabled` so a missing storage key (UI `_ga` defaults the checkbox to ON) sent type 90 with `icons:[]` → native drew nothing. MV2 used `0==b.enabled`. FIX: match MV2; set `enabled:true` when generating; re-push after configLoaded + on `mouseGest` storage changes; page `_6t` routed to SW (`cmd:gestureDisplay`); `file46.css` `@font-face gestureDirs` (MV2 loaded it from background `file63.html`). mh_test A7c + B53. NO bundle rebuild |
