...
/* AC-MV3 FIX (2026-09-16): fullscreen toggle read `_cd[g].state` (SW window
   cache, 1500ms `_Fu` enum). After entering fullscreen the cache still said
   "normal", so the next gesture re-issued state:"fullscreen" (no-op) until
   the enum refreshed — same class as pin/mute B52. Toggle now windows.get
   first and writes f.state from the update result (restore `_oa` already
   did). Chrome may still drop a second state change while the animation
   is in flight. */
var _4d=_cg(function*(b,d,a){const c=yield e=>_Lk(_uh,!1,e);for(let g of _yl(_qd(b))){const w=yield e=>_Yk.windows.get(g,e);if(_Aw()||!w)continue;let f=_cd[g];if(!f)continue;const h="fullscreen"==w.state;!h&&w.state.in("normal","maximized")&&(f.prevState=w.state);if(a.mode!=h){const u=yield e=>_Yk.windows.update(g,{state:h?f.prevState||"normal":"fullscreen"},e);!_Aw()&&u&&(f.state=u.state,u.state.in("normal","maximized")&&(f.prevState=u.state))}else f.state=w.state}c&&(yield e=>_Lk(_uh,!0,e));d(b)});function _hh(b,d,a){switch(a.unit){case "px":return _ey(b,a.baseMonitor,a);case "tile":return _Xe(b,d,a.baseMonitor,a);case "%":return _Xr(b,a.baseMonitorId,a)}return b}
...
function _0p(a={}){a=a.triggers||{};let b=a.preset||"middleButton";"other"!=b&&a.add(_Vi[b].value);a.timeout=1E3*(a.timeout||1.5);return a}function _Jp(a={}){a=a.sc();null==a.dirPrecision&&(a.dirPrecision=4);"stepSize"in a&&(a.stepSize=Math.max(3,a.stepSize));"dirChangeSens"in a&&(a.dirChangeSens=_fu(a.dirChangeSens,1,9)+1);_Lk(_Ei,a)}
